...
url = "https://53.220.250.2/api/v0"

client = ClientLibrary("https://53.220.250.2", "admin", os.getenv("CML_PASS"), ssl_verify=False)
...
def cml_create_lab_topology(tag,nodes):
    ...
    for k,v in nodes.items():
        for files in read_files:
            if k in files:
                position = 50
                node = lab.create_node(k, v, -100, 200 + position)
                node.config = k
